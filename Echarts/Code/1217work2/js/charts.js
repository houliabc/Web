...
// 处理动指标数据（柱状图）
function barProcess(data, Allcategories) {
    const categoryStats = {};
    ...
    data.forEach(item => {
        ...
        const category = Allcategories[item.分区];
...
// 处理用户行为转化数据（热力图）
function heatmapProcess(data, Allcategories) {
    // 定义排名区间
    const rankRanges = [
...
// 处理分区分布数据（气泡图）
function bubbleProcess(data, Allcategories) {
    const categories = {};
    data.forEach(item => {
        // 跳过空分区
        if (!item.分区) return;
...
// 处理综合评估数据（雷达图）
function radarProcess(data, Allcategories) {
    const categoryStats = {};
    data.forEach(item => {
        const category = Allcategories[item.分区] || '其他';
        if (!categoryStats[category]) {
            categoryStats[category] = {
...
$.getJSON('bilibili_popular.json', function(data) {
    ...
    // 处理数据
    const barData = barProcess(data, Allcategories);
    const heatmapData = heatmapProcess(data, Allcategories);
    const bubbleData = bubbleProcess(data, Allcategories);
    const radarData = radarProcess(data, Allcategories);
