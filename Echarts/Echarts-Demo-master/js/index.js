    ...
    series: [
      {
        ...
        data: [[1200, 1400, 1600, 1800, 1600, 1200]],
        symbol: 'none',
        itemStyle: {
          color: '#2f89cf'
        },
      ...
      },
      {
        ...
        data: [[1400, 1600, 1800, 1600, 1400, 1400]],
        symbol: 'none',
        itemStyle: {
          color: '#00c292'
        },
        areaStyle: {
          color: 'rgba(0,194,146,0.3)'
        }
      }
    ]
  };
...
(function() {
  ...
  // 5.添加切换按钮到HTML（确保在图表初始化后添加）
  $('.pie h2').append('<a href="javascript:;" class="age active">年龄</a><a href="javascript:;" class="education">学历</a>');

  // 6.绑定点击事件
  $('.pie h2 a').on('click', function() {
    const type = $(this).attr('class').split(' ')[0];
    option.series[0].data = pieData[type];
    
    // 更新按钮样式
    $('.pie h2 a').removeClass('active');
